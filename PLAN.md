...
2. **AD integration (existing)**: keep `_test/ADClientTest.php`,
   ...
   fixture from `_test/docker-compose.yml` on `localhost:7389`; skip
   cleanly when unreachable. With AD now inheriting from `LDAPClient`,
   these tests also exercise the universal code path — strong
   regression coverage.
...
1. **AD regression** — with `directory_type=ad` (default), run all of
   ...
   the Samba AD-DC fixture from `_test/docker-compose.yml`. All must
   pass. (After step 3, these simultaneously exercise the new
   `LDAPClient` base path.)
